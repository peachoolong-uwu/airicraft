...
HTTP429 during a gameplay planner request preserves the current role/session and waits for Retry-After (30seconds when absent or invalid), then retries without consuming the dialogue degradation threshold. New guidance cannot bypass the provider cooldown. Other provider errors retain their existing behavior; compaction uses its separate recovery path.
