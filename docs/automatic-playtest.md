...
For new recordings, the files in the table are staging names. On publication they move into `PLAY/extensions/airicraft.playtest/`: JSONL streams are gzip-compressed, large JSON/log files are compressed too, and `world-save/` becomes `world-save.zip`. Inspect the `manifest.json` for exact asset paths. The whole Play can be copied independently; its evidence has no dependency on the run index or worker directory. Earlier recordings remain unchanged.

Point the recorder viewer's `[paths].artifacts` at the absolute `automatic_playtest` output root, start its catalog server, and select a Play. The **Playtest** panel opens automatically. It provides run status, the planner bug report, screen video with a Server tick slider, and bounded/searchable evidence-file previews and downloads. Complete Plays also add a playtest track to the timeline. The panel can inspect an incomplete Play directly from Resources even when replay validation prevents adding it to the timeline. Automatic playtests enable direct framebuffer sampling at up to one frame per server second (640×360 RGB; identical frames are skipped). Structured observations are streamed to disk each tick. Sampled JPEGs feed a background H.264 encoder and are omitted from that JSONL stream. `screen.mp4` uses independently flushed fragments with a two-frame keyframe interval; it can retain decodable earlier fragments after a crash. Identical images are held in the video during quiet periods. `screen-frames.jsonl` records each sampled image's video time and debug clock, and the published `video-index.json` maps those times to authoritative Server ticks. The server thread records both clock coordinates together in `recording-start.json`. The viewer holds the last captured image rather than assuming video time equals ticks divided by 20. No audio is captured. The completion record and summary flag any detected sequence gaps. Reported incidents also include a final full screenshot.

Recorder Play supplies the structured recorded world/packet state for replay and inspection; it covers client-visible capture, not an authoritative server checkpoint at every historical tick. `world-save.zip` is one saved checkpoint, not a sequence of historical saves. Normal disconnect can add a short tail to the Play after a report tick; use `pause-verification.json` and `world-save.json` to locate the incident. Extracting the checkpoint into a separate saves directory lets an investigator resume from there. A failed run before player join remains an ordinary harness archive because no Recorder Play identity exists yet. Publishing a crash extension never fills in recorder metadata end fields or repairs an unfinished replay.
...
- Role `playtest`, schema `airicraft.playtest.v1`, `playtest.json`: version 1, run outcome, observed timeline bounds, debug clock offset, optional bug report, and file sizes. An unfinished Play's observed bounds are evidence coverage, not a completion marker.
- Role `video_index`, schema `airicraft.screen-index.v1`, `video-index.json`: ordered `{serverTick: decimal-string, videoSeconds: number}` samples and `complete`. Only samples covered by retained video packets are included.
- Role `screen_video`: fragmented H.264/YUV420p MP4 from the live framebuffer.
- Roles `observations`, `world_checkpoint`, `recorder_scratch`, and `evidence`, schema `airicraft.evidence.v1`: retained producer files with their original evaluation/dashboard payloads; gzip decompresses to the original bytes, including a possible incomplete final line. Original client ticks, debug tick IDs, and wall times stay explicitly named. Translate a dashboard `serverTickId` by subtracting `debugTickOffset`; do not reinterpret an agent `tick` as a Server tick.
