...
Tool turns remain bounded at20calls. When a follow-up proposes more tools than fit, the runtime preserves completed tool results, discards the extra proposal, and yields with no visible chat. A checkpoint in the retained conversation identifies the boundary. Existing goal/delegation continuation starts a fresh turn when work is idle; it does not mark the task complete or change the decision owner. Exhaustion is not a malformed tool call and does not trigger format repair or degraded mode.
