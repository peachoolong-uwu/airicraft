...
The acquisition executor selects loaded targets, finds an interaction position, navigates, breaks the exact source and pursues matching drops. It can clear leaves obstructing a source. It uses eye-based interaction reach. Baritone receives exact navigation goals; production mining dispatch no longer invokes its general `mineByName` process. The existing explicit underwater-harvest executor retains its separate breathing and movement behavior.

A source with no usable observed work position is skipped. Failed or stalled approaches are excluded for that attempt; movement has a 240-tick limit and an 80-tick no-progress limit. Breaking has a 240-tick limit, pickup settling is bounded, and the whole attempt has a 2,400-active-tick budget. A partial result does not satisfy an inventory goal. Missing targets, failed pickup and scope exits return terminal evidence to the planner; constraints are never silently widened. Scoped acquisition does not perform speculative excavation to discover buried ore.

Current phase, selected target, inventory count and rejected-target reason appear in task execution snapshots and the flight recorder. These details are available for diagnosis without requiring System 2 to choose every block or movement.
