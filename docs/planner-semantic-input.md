...
- Keep canonical decision history, dispatch metadata and event cursors structured. Format model request text without modifying that history, tool-call pairing, images, native executor identities or persistence. Retry rendering is deterministic and does not depend on a previous request having been seen.
- Unknown fields in recognized JSON envelopes pass through as additional structured fields. Unrecognized tools or text formats remain unchanged. Do not invent a generic lossy JSON-to-prose transformation.
