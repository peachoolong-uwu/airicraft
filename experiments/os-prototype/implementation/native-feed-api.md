...
Sampled server ticks do not prove continuous process eligibility. Item projections deliberately publish `eligibleTicks: null`. The optional [native progress scopes](progress-api.md) connect explicit chunk/passive-entity counters to wait deadlines. They use distinct configured scopes and grants. Separately, [native availability evidence](availability-api.md) now feeds `work.progress` for adapters explicitly covered by unchanged material and ordinary authority. Host allocation changes, stale captures and changed generations break that continuity. Only bound-container work is currently covered; other operation scopes and retained contexts remain separate integrations.
