...
Authority responses also expose `generation` when the lease is null. Recovery may establish non-admission only when the same epoch and generation are fenced, both lease and active receipt are explicitly null, and the admission high-water mark is below the request sequence. A different generation, a mark at or beyond the sequence, missing authority fields, or an active owner leaves an unknown receipt unresolved. This additive field and retained admission rejection are covered by 33 focused native tests. The corrected build also completed a live broker crash with a retained partial-transfer receipt; the never-admitted recovery branch remains simulated.
