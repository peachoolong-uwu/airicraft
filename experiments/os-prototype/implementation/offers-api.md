...
Declarations are keyed by their complete operation, arguments and context (null or a configured string key under the [host context contract](host-context-api.md)). Identical entries collapse. Unchanged entries retain their work ID across evaluations, including reordered arrays, rather than acquiring new queue positions. Distinct invocations retain distinct declarations and root-owned fairness; duplicate offers do not multiply a root's age or service credit. This is not semantic merging of different argument shapes.
