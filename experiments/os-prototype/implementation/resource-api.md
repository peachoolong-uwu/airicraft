...
These views are scheduling inputs. Even a known shortage remains a hint until the coordinator obtains a fresh native frame and admits a valid atomic bundle. The work scheduler still needs to connect those proposals to native readiness, combined offer limits, admission and inspection/context entry. This service does not poll or reopen an empty chest just because a demand exists.
