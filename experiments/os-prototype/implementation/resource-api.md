...
These views are scheduling inputs. Even a known shortage remains a hint until the coordinator obtains a fresh native frame and admits a valid atomic bundle. The work scheduler must combine target deficits and finite demands, choose applicable bounded supply rules, and schedule inspection/context entry as needed. That automatic selection and the domain operations remain subsequent integration work. This service does not poll or reopen an empty chest just because a demand exists.
