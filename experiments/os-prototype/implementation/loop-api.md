...
The [resource service](resource-api.md) implements `target` and `demand` when supplied to the loop. The optional [work service](work-api.md) registers finite `work` requests and waits for coordinator-verified physical completion. Absent configuration returns `service_unavailable`; `worker` remains unavailable. Retained-context/fishing dispatch and native eligibility production remain separate integrations. Offer definitions require both `work` and `observations`; an offer child rejects before allocation if either is absent. There is no generic callback/plugin dispatcher that could bypass these boundaries.
