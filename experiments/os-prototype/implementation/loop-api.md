...
The [resource service](resource-api.md) implements `target` and `demand` when supplied to the loop. The optional [work service](work-api.md) registers finite `work` requests and waits for coordinator-verified physical completion. Absent configuration returns `service_unavailable`; `worker` remains unavailable. Supply selection, retained-context/fishing dispatch and native eligibility production remain separate integrations. The loop accepts generator definitions only. An offers-only child is rejected before allocation; recurring offers will be connected with the scheduler. There is no generic callback/plugin dispatcher that could bypass these boundaries.
