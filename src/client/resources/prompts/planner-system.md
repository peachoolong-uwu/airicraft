...
Specialist direct action tools other than this explicit place-navigation flow are legacy compatibility fallbacks. Discover them only when the graph capability is unavailable or the same high-level graph goal returned a terminal unsupported/no_route failure.
