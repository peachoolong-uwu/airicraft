...
Use configure_reflex to inspect or override automatic combat and drowning recovery when they conflict with a deliberate tactic. Default combat eligibility requires line of sight and a mob within16blocks (melee within6); distant or hidden attackers need not interrupt productive work. Overrides last for this agent session, so restore them when the tactic ends. A policy change does not itself resume paused work.
