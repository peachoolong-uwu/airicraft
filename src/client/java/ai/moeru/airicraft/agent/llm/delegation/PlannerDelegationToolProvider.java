...
public final class PlannerDelegationToolProvider implements PlannerToolProvider {
	...
	@Override public String promptInstructions() {
		return role == Role.CONTROLLER
			? "You are the non-thinking main controller. Handle routine gameplay directly. For a complex multi-step problem, call delegate_task with a bounded task and observable success criteria. The thinking planner takes over gameplay until it returns. Its tool result includes observed actions and final facts; review these before choosing the next step."
