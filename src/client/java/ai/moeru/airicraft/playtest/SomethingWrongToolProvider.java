...
public final class SomethingWrongToolProvider implements PlannerToolProvider {
	...
	@Override public String promptInstructions() {
		return "This is an automatic playtest. Call something_wrong when observed behavior suggests a bug in Airicraft's tools, execution, observations or harness. Describe what you tried, expected behavior, actual behavior, and any useful work IDs in ordinary language. You do not need a root cause or certainty. Ordinary survival difficulty, missing materials, an unreachable target or your own mistaken plan alone are not interface bugs. If pathfinding fails repeatedly for the same intended route (including PATH_STUCK/navigation_stuck), call something_wrong with the attempted targets and work IDs instead of retrying indefinitely; report the failure even if the root cause is unclear. A report pauses the game and preserves the recording for later review; do not continue gameplay after reporting.";
	}
